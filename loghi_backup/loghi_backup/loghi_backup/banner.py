...
def calcola_gradient(col1, col2, height):
    return [tuple(int(col1[i] + (col2[i] - col1[i]) * (j / height)) for i in range(3)) for j in range(height)]

def aggiungi_gradient(draw, area, col1, col2):
    gradient = calcola_gradient(col1, col2, area[3])
    for i in range(area[3]):
        draw.line([(area[0], area[1] + i), (area[0] + area[2], area[1] + i)], fill=gradient[i])
...
def aggiungi_logo_o_iniziali(img, draw, squadra, posizione_x, posizione_y, larghezza, altezza, dimensione_logo=(100, 100)):
...
def genera_immagine(params):
    ...
    if len(colori_casa) == 1:
        aggiungi_gradient(draw, (0, 0, larghezza // 2, altezza), colori_casa[0], colori_casa[0])
    else:
        aggiungi_gradient(draw, (0, 0, larghezza // 2, altezza), colori_casa[0], colori_casa[1])

    # Colori squadra trasferta
    if len(colori_trasferta) == 1:
        aggiungi_gradient(draw, (larghezza // 2, 0, larghezza // 2, altezza), colori_trasferta[0], colori_trasferta[0])
    else:
        aggiungi_gradient(draw, (larghezza // 2, 0, larghezza // 2, altezza), colori_trasferta[0], colori_trasferta[1])
